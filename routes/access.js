...
const { neo4jQueryWrapper, validateFields, disconnectSessionSockets } = require("../utils/utils");
...
router.post("/logout", authenticationCheck, (req, res, next) => {
  ...
  neo4jQueryWrapper("MATCH (u:User{sessionUserID:$sessionUserID})-[:JOINED]->(c:Conversation) WITH u, collect(c) AS cl RETURN u, cl", {
    sessionUserID
  })
    .then(({records: [record]}) => {
