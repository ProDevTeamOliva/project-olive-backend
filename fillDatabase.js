...
const fillDatabase = async () => {
    ...
    for(let i=0; i<users.length; i++) {
        ...
        for(let j=0; j<user.friends.length;j++) {
            ...
            try {
                const responseFriend = await axios.post(`/user/${friend.id}/friend`, {}, config)
                console.log(responseFriend.data);
            } catch {
                const responseAccept = await axios.post(`/user/${friend.id}/accept`, {}, config)
        ...
        for(let j=0; j<likes.length;j++) {
            ...
            for(let k=0; k<posts.length;k++) {
                ...
                const responseLike = await axios.post(`/post/${post.id}/like`, {}, config)
